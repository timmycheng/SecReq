/* NetBox 互通(#271 重设计): 单向 ETL 同步 —— SecReq → NetBox 只推不拉。
   配置区(地址/Token/系统类型/字段映射) + 同步调度(开关/周期/立即同步) + 同步历史。
   其他角色与页面零感知: 系统清单/向导不出现任何 NetBox 入口, 同步在后台执行。 */
import { useCallback, useEffect, useRef, useState } from 'react'
import { Alert, Button, Form, Input, InputNumber, Space, Switch, Table, Tag, Typography, message } from 'antd'
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      <Alert
        ...
        message="单向同步: SecReq → NetBox(#271)"
        description={
          'NetBox 作为后备资产库, 平台把系统与基础设施(server/network)信息定期/手动推送到 NetBox 存档, ' +
          '只推不拉; 系统与基础设施的填报永远在本平台维护, 不从 NetBox 导入。设备同步使用 secreq-* ' +
          '引导对象(机房/角色/机型), 缺失时自动创建。'
        }
      />
      ...
      <Typography.Title level={5} style={{ marginTop: 24, marginBottom: 8 }}>同步历史</Typography.Title>
