...
界面优化批次(milestone 全部 9 个 issue), 纯前端体验改善为主, 无数据迁移; 仅新增 拓扑画布相关表与端点(新增均带回退)。测试 229 个通过(另有 5 个 `xfail(strict=True)` 护栏用例)。
...
管理端能力补全与工程质量批次(milestone 全部 7 个 issue), 无数据迁移、无破坏性 API 变更(均为新增端点/字段, 带回退)。测试 224 个通过(另有 5 个 `xfail(strict=True)` 护栏用例), 并新增 Playwright E2E 主链路接入 CI。
