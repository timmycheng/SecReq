...
def save_infra_topology(session: Session, project_id: int, env: str,
                        zones: list[dict], links: list[dict],
                        layout: dict, assets: list[dict]) -> dict:
    """按环境整卷保存拓扑(设备/区域/连线/布局), 幂等(#93)。

    - 设备按 uid upsert(仅该 env), zone_uid 解析为 zone 主键;
    - 区域按 uid upsert, 提交缺失的删除(其设备 zone_id 置空);
    - 连线整卷重建(uid 引用, 不携带自身主键语义);
    - 布局单行 upsert, 不进规则引擎。
    """
    from models import InfraLayout, InfraLink, NetworkZone

    # ── 区域 upsert ──
    existing_zones = {z.uid: z for z in session.query(NetworkZone).filter_by(project_id=project_id, env=env)}
    zone_by_uid: dict[str, NetworkZone] = {}
    for z in zones:
        zone = existing_zones.get(z["uid"])
        if zone is None:
            zone = NetworkZone(project_id=project_id, uid=z["uid"], env=env, name=z["name"])
            session.add(zone)
        else:
            zone.name = z["name"]
        zone_by_uid[z["uid"]] = zone
    for uid, zone in existing_zones.items():
        if uid not in zone_by_uid:
            session.delete(zone)
    session.flush()

    # ── 设备 upsert(仅该 env) ──
    existing_assets = {a.uid: a for a in session.query(InfraAsset).filter_by(project_id=project_id, env=env)}
    submitted_uids: set[str] = set()
    for item in assets:
        uid = item.get("uid") or gen_uid()
        zone_uid = item.get("zone_uid")
        zone_id = zone_by_uid[zone_uid].id if (zone_uid and zone_uid in zone_by_uid) else None
        attrs = {
            "asset_type": item.get("asset_type", "server"),
            "name": item.get("name", ""),
            "env": env,
            "ip": item.get("ip"), "owner": item.get("owner"),
            "holds_sensitive": bool(item.get("holds_sensitive")),
            "cpu_cores": item.get("cpu_cores"), "memory_gb": item.get("memory_gb"),
            "disk_gb": item.get("disk_gb"), "os": item.get("os"),
            "quantity": item.get("quantity"), "purpose": item.get("purpose"),
            "zone_id": zone_id,
        }
        if uid in existing_assets:
            row = existing_assets[uid]
            for key, value in attrs.items():
                setattr(row, key, value)
        else:
            row = InfraAsset(project_id=project_id, uid=uid, **attrs)
            session.add(row)
        submitted_uids.add(uid)
    for uid, row in existing_assets.items():
        if uid not in submitted_uids:
            session.delete(row)

    # ── 连线整卷重建 ──
    session.query(InfraLink).filter_by(project_id=project_id, env=env).delete(synchronize_session=False)
    for lk in links:
        session.add(InfraLink(
            project_id=project_id, env=env,
            source_uid=lk["source_uid"], target_uid=lk["target_uid"],
            label=lk.get("label"),
        ))

    # ── 布局单行 upsert ──
    layout_row = session.query(InfraLayout).filter_by(project_id=project_id, env=env).first()
    if layout_row is None:
        session.add(InfraLayout(project_id=project_id, env=env, layout=layout))
    else:
        layout_row.layout = layout

    session.commit()
    return {"zones": len(zone_by_uid), "assets": len(submitted_uids), "links": len(links)}
