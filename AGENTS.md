...
- 应用外壳(#280 改版): `frontend/src/App.tsx` 深蓝分组侧边菜单(工作台/安全管理/平台设置) + 顶部面包屑; 「平台设置」组页面(系统管理/备案管理/知识库管理/用户管理/LDAP 对接/LLM/Netbox/日志审计/更新日志)仅安全角色可见, 独立页面组件在 `frontend/src/ui/`。系统管理仍是单页 Tabs(系统设置/漏洞库): 外壳 `frontend/src/ui/AdminPage.tsx`, Tab 组件在 `frontend/src/ui/admin/`(React.lazy)。新增平台设置页 = ui/ 新组件 + App.tsx MENUS/路由注册 + `api.ts` 方法 + 后端端点(写端点挂 `Depends(require_security)`), 前端没有权限注册步骤。工作台聚合数据走 `GET /api/meta/dashboard`(按数据权限过滤)。
