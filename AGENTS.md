...
- 质量门禁本地命令: `uv run pytest tests -q`、`uv run ruff check .`、`cd frontend && npm run lint && npm run build`(build 含 tsc 类型检查); 后端依赖经 uv 锁定(`pyproject.toml` 声明 + `uv.lock` 锁树入库, 改声明必须 `uv lock` 连锁文件一起提交), 测试夹具在 `tests/conftest.py`: `api` 默认开发身份, `api_as(api, "sec_chen")` 切安全身份。
