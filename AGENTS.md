...
- 应用外壳(#280 改版): `frontend/src/App.tsx` 深蓝分组侧边菜单(工作台/安全管理/平台设置) + 顶部面包屑; 「平台设置」组页面(系统设置/漏洞库/备案管理/知识库管理/用户管理/LDAP 对接/LLM/Netbox/日志审计/更新日志)仅安全角色可见, 独立页面组件在 `frontend/src/ui/`; 系统设置与漏洞库原为 AdminPage 单页 Tabs, #285 起拆为两个独立页面(路由 /admin 与 /vulndb), 表单/内容组件仍在 `frontend/src/ui/admin/`(React.lazy 按需加载)。新增平台设置页 = ui/ 新组件 + App.tsx MENUS/路由注册 + `api.ts` 方法 + 后端端点(写端点挂 `Depends(require_security)`), 前端没有权限注册步骤。工作台聚合数据走 `GET /api/meta/dashboard`(按数据权限过滤)。
- 平台鉴权五角色(#309): pm 项目经理 / dev_admin 开发管理员(两开发侧) / security_admin 安全管理员(安全侧, 单步评审裁定) / sys_admin 系统管理员 / auditor 审计员(系统层面); 角色枚举与分组常量(FULL_VISIBILITY_ROLES/SECURITY_SIDE_ROLES/PLATFORM_ADMIN_ROLES/WRITE_WIZARD_ROLES/SYSTEM_WRITE_ROLES/REVIEW_SUBMIT_ROLES)在 `shared/constants.py`; 无权限点/权限表(`models/permission.py` 是项目内"权限矩阵"设计器的产物, 不是平台 RBAC); 全局 auth_guard 挂在 `main.py`, 开放前缀与 `require_login`/`require_write_roles` 在 `routers/common.py`。
...
本仓库所有改动一律走 GitHub 标准流程, **禁止直推 main**(分支保护对管理员同样生效):先开 issue(模板 + `type:`/`priority:` 标签, 版本批次挂对应 milestone)→ 从最新 main 切分支, 命名 `<type>/<issue号>-<slug>`(如 `fix/18-schema-upgrade`)→ 提交与 PR 标题用约定式前缀 + 中文描述(如 `fix(认证): 初始密码环境变量化`)→ PR 正文写 `Closes #N` → 等 CI 必查 job(后端检查 / 前端检查)全绿后 squash 合并。

存在并行开发(多个 issue/分支同时推进)时, 尽量为每个分支创建独立的 git worktree 进行开发(如 `git worktree add ../secreq-wt-<issue号> <分支名>`), 各 worktree 独立切分支、跑测试与构建, 互不干扰; 避免在唯一工作区来回切分支导致进行中的改动互相踩踏。零散小改动可不开 worktree, 直接在主工作区完成。

发版仅由 tag 触发:打 tag 前必须跑 `python scripts/check_version.py vX.Y.Z`, 保证 tag、`main.py` 的 version、`pyproject.toml` 的 version、CHANGELOG 章节四者一致;版本批次以 milestone 管理,milestone 全部关闭才具备发版条件。bump `pyproject.toml` 版本号本身就是改声明:发版 PR 内必须 `uv lock` 连锁文件一起提交(锁文件记录项目自身版本, 漏锁会被 CI 的 `uv lock --check` 拦截)。
