...
4. 评审/审批页: 内容区 + 右侧固定「评审操作面板」(提交评审/裁定/意见输入; #309 单步评审, 无独立终审)——产物页与评审页一律用这个模式
5. 工作台: 指标卡 + 图表 + 最近评估 + 快捷入口, 数据走 `/api/meta/dashboard` 聚合端点(#280)

> 外壳与菜单(#280): 应用外壳在 `App.tsx` —— 深蓝分组侧边菜单(工作台/安全管理/平台设置) + 顶部面包屑与用户区, 内容区 `maxWidth: 1440` 居中。「平台设置」组(系统设置/漏洞库/备案管理/知识库管理/用户管理/LDAP 对接/LLM/Netbox/日志审计/更新日志)仅安全管理员/系统管理员可见(#309); 新增页面 = `ui/` 下新组件 + `App.tsx` MENUS/路由注册 + `api.ts` 方法 + 后端端点。
...
- 中文文案统一用词: 「提交评审」「退回整改」「评审通过」(#309 单步评审, 无「终审/复审」表述), 禁止同义混用(不要出现「送审」「打回」「过审」)
