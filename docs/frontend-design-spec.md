...
> 外壳与菜单(#280): 应用外壳在 `App.tsx` —— 深蓝分组侧边菜单(工作台/安全管理/平台设置) + 顶部面包屑与用户区, 内容区 `maxWidth: 1440` 居中。「平台设置」组(系统管理/备案管理/知识库管理/用户管理/LDAP 对接/LLM/Netbox/日志审计/更新日志)仅安全角色可见; 新增页面 = `ui/` 下新组件 + `App.tsx` MENUS/路由注册 + `api.ts` 方法 + 后端端点。

> 归属说明(#234): 系统管理(`ui/AdminPage.tsx`)是 Tab 容器外壳(系统设置/漏洞库), 本身不属于五种布局模式; 每个 Tab 的内容区按其内容归入上述模式之一。
