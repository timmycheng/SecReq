...
开 issue(模板 + type:/priority: 标签, 版本批次挂 milestone)
  ...
  → CI 必查 job 全绿(后端检查 / 前端检查; E2E 主链路随 PR 运行, docs-only 改动整体跳过)
...
- `ci.yml` 在 PR 与 push main 时触发: PR 跑全量(后端检查 = Lint + 锁校验 + pytest / E2E 主链路 / 前端检查 = Lint + 类型检查), push main 只保留后端检查兜底合并交互; docs-only 改动(全部为 md 或 docs/**)经「变更探测」job 判定后整体跳过, 被跳过的必查 check 视同通过。同分支新提交自动取消旧运行。
