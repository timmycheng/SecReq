...
- **8 步建项向导**:项目定级(基本信息/外部系统连接清单、定级问卷内联、定级后即时
  预览密码策略与合规基线)→ 功能清单 → 数据字典 → 权限矩阵 → API 接口 →
  基础设施(架构图上传/服务器规格/网络设备清单)→ 组件许可(SBOM)→ 确认生成;项目编码自动生成(`XM2026-001`)。
...
- **平台设置(安全角色)**:系统设置(项目编号规则/密码策略基线/定级题库)、漏洞库
  (版本/生态覆盖/完整性校验)、知识库/定级题库可视化编辑(写回 YAML 自动备份+全量校验)、
  OpenAI 兼容大模型接入配置(含测试连接)、更新日志、用户管理、审计日志。
- **认证与数据权限**:账密登录(pbkdf2 + Bearer 会话 12h + 登录失败锁定),全接口
  鉴权;**开发只能看到/操作自己创建的项目,安全全量可见**(越权一律 404)。
- **系统清单与评估继承**:定级备案/系统实体台账页,项目可归属系统;新建项目可整卷
...
摆渡完成后可在**平台设置 → 漏洞库**查看库版本、生态覆盖与记录数, 并点
...
| sec_admin | 安全 | 查看全部项目、平台设置全部页面(系统设置/漏洞库/知识库/题库/策略/LLM/用户/审计/备案/Netbox/更新日志) |
...
│  ├─ system_service.py   # 系统清单(定级备案/系统实体, 项目归属系统)
